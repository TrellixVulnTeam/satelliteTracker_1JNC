...
def timeInfo():
    with open('static/d3/time.txt', 'r') as f:
        timeOld = f.read()
    ...
    if timeNow > timeOld:
        ...
        with open('static/d3/time.txt', 'w') as f:
            f.write(str(timeOld))
        ...
        with open('static/d3/spacecraft.txt', 'w') as f:
...
def comp():
    TLEs = readTLEfile("static/d3/spacecraft.txt")
    numSats = int(len(TLEs) / 3.0)
    with open('static/d3/satelliteData.csv', 'w') as f:
        f.write('name,')
        f.write('latitude,')
        f.write('longitude,')
        f.write('elevation,')
...
def getSunData():
    ...
    with open('static/d3/sun.csv', 'w') as f:
        f.write('lat,')
        f.write('lon,\n')
        for j in range(len(dList)):
...
def writeGroundSites():
    with open("static/d3/orbitLength.csv", 'w') as f:
        f.write("len,\n")
        f.write(str(m * hr))
    with open("static/d3/groundData.csv", 'w') as f:
